...
from pi.utility import MyUtility
...
class MyBaiduCrawler():
    ...
    def parse_tieba(self, keyword):
        ...
        for t in html.xpath('//li[contains(@class, "j_thread_list")]'):
            ...
            for i in t.xpath('.//img[contains(@class,"threadlist_pic")]'):
                ...
                # this is full size pic, has to save locally first. Link to
                # Baidu won't work.
                imgs.append(i.get('bpic'))
    ...
    def parser(self, params):
        ...
        # school name can be changed outside this request, so we take
        # precaution here!
        try:
        ...
        # save results to DB
        highpoints = MyUtility().highpoints
        for t in results:
            ...
            if ':' in t['last_timestamp']:
                ...
                now = timezone.now()
                post_timestamp = dt(
                    now.year, now.month, now.day, int(tmp[0]), int(tmp[1]))
                post_timestamp = pytz.timezone(
                    timezone.get_default_timezone_name()).localize(post_timestamp)
            ...
            if len(data) > 1:
                ...
                data = MyBaiduStream(url_original=t['url'], school=school)
            ...
            data.name = highpoints.sub(u'', t['title'])
            data.description = highpoints.sub(u'', t['abstract'])
            ...
            for img_url in t['imgs']:
                for a in Attachment.objects.filter(source_url=img_url):
                    a.delete()  # exist
                ...
                img_data = None
                try:
                    img_data = self.retriever.request(img_url)
                except:
                    self.logger.error('retrieve img failed: %s' % img_url)
                if img_data:
                    tmp_file = NamedTemporaryFile(suffix='.jpg', delete=False)
                    tmp_file.write(img_data)
                    if Attachment.objects.filter(source_url=img_url).exists():
                        continue
                    else:
...
class MyTrainCrawler():
    ...
    def parser(self, train_id):
        ...
        if not html.xpath('//div[@class="s_hd"]/span'):
            return
    ...
    def parser(self, city, province_id):
        ...
        if wiki:
            ...
            for element, attribute, link, pos in wiki[0].iterlinks():
                if attribute == "href":
                    element.set(
                        'href', 'http://zh.wikipedia.org' + element.get('href'))
            ...
            self.logger.info('Found nothing: ' + city)
...
class MyJobCrawler():
    ...
    def parser(self, keyword):
        ...
        url = 'http://search.51job.com/jobsearch/search_result.php?fromJs=1&jobarea=000000,00&funtype=0000&industrytype=00&keyword=%s&keywordtype=2&lang=c&stype=1&postchannel=0000&fromType=3' % urllib.quote(
            major.name.encode('utf-8'))
        content = self.http_handler.request(url)
        html = lxml.html.document_fromstring(clean_html(content))
        ...
        for result in result_list:
            for job_name in html.xpath('.//a[contains(@class,"jobname")]'):
                ...
                jobs.append(
                    (job_name.text_content().strip(), job_name.get('href')))
            ...
            for index, attributes in enumerate(html.xpath('.//tr[contains(@class,"tr1")]/td[contains(@class,"td1234")]')):
                ...
                for tmp in [t.strip() for t in attributes.text_content().strip().split('|')]:
                    if tmp.startswith(u'学历要求'):
                        req_degree = tmp[5:].strip()
                    elif tmp.startswith(u'工作经验'):
                        req_experience = tmp[5:].strip()
                    elif tmp.startswith(u'公司性质'):
                        co_type = tmp[5:].strip()
                    elif tmp.startswith(u'公司规模'):
                        co_size = tmp[4:].strip()
...
class MySogouCrawler():
    ...
    def parser(self, id):
        ...
        url = 'http://weixin.sogou.com/weixin?type=1&query=%s&fr=sgsearch&ie=utf8&_ast=1430486528&_asf=null&w=01029901&cid=null' % urllib.quote(
            school.name.encode('utf-8'))
        ...
        if not total_page:
            total_page = 2  # we are counting from 1

        for page in range(1, total_page):
            url = "http://weixin.sogou.com/weixin?type=1&query=%s&fr=sgsearch&ie=utf8&page=%d" % (
                urllib.quote(school.name.encode('utf-8')), page)
            ...
            for result in html.xpath('//div[contains(@class,"results")]/div'):
                ...
                if school.name not in name:
                    continue

                source_url = result.get('href')
                account_id = result.xpath(
                    './/h4')[0].text_content().strip().split(u'：')[1]
                description = result.xpath(
                    './/span[contains(@class,"sp-txt")]')[0].text_content().strip()
                wx, created = MyWeixinAccount.objects.get_or_create(
                    account=account_id, name=name)
                wx.school = school
                wx.description = description
                wx.sg_url = 'http://weixin.sogou.com%s' % source_url
                wx.save()
                if created:
                    self.logger.info('%s added' % name)
                else:
                    self.logger.info('%s updated' % name)

                # we already have the barcode
                if len(wx.attachments.all()):
                    continue

                # images
                icon_url = result.xpath(
                    './/div[contains(@class,"img-box")]/img')[0].get('src')
                img_data = None
                try:
                    img_data = self.retriever.request(icon_url)
                except:
                    self.logger.error('retrieve img failed: %s' % icon_url)
                if img_data:
                ...
                img_data = None
                try:
                    img_data = self.retriever.request(barcode_url)
                except:
                    self.logger.error('retrieve img failed: %s' % icon_url)
                if img_data:
        ...
        # STEP 2: get articles
        url = "http://weixin.sogou.com/weixin?type=2&query=%s&fr=sgsearch&ie=utf8&tsn=2&interation=" % urllib.quote(
            school.name.encode('utf-8'))
        ...
        for result in html.xpath('//div[@class="results"]/div'):
            ...
            if tweet_summary:
                tweet_summary = tweet_summary[0].text_content()

            img_url = ''
            img = result.xpath('.//div[@class="img_box2"]//img')
            if img:
                img_url = 'http://' + img[0].get('src').split('http://')[-1]
...
class MyHudongWikiCrawler():
    ...
    def parser(self, school_id):
        ...
        if wiki:
            ...
            self.logger.info('Found nothing: ' + school.name)

        summary_table = html.xpath('.//div[@id="datamodule"]//table')
        if summary_table:
            school.hudong_summary_table = lxml.html.tostring(summary_table[0])
        toc = html.xpath('.//fieldset[@id="catalog"]')
        if toc:
            school.hudong_toc = lxml.html.tostring(toc[0])
        school.save()
    ...
    def parser(self, id):
        ...
        try:
            school_name = html.xpath(
                '//h1[@id="page-title"]')[0].text_content().strip()
        except:
            return  # we don't have a school name, could be "page not found"
        ...
        for info in html.xpath('//div[@id="school-info"]'):
            ...
            if campus:
                campus = campus[0].text_content().strip()
            else:
                campus = ''
            ...
            for p in html.xpath('.//p')[-3:-1]:
                ...
                if 'Mailing' in tmp[0]:
                    ...
                    if zip_pat.search(physical_zip):
                        ...
                        try:
                            physical_zip = MyZip.objects.get(
                                zipcode=physical_zip)
                        except:
                            physical_zip = None
            ...
            for school_type in html.xpath('.//div[@id="school-type"]'):
                if html.xpath('.//span[contains(@class,"f-1")]'):
                    f_1 = True
                else:
                    f_1 = False

                if html.xpath('.//span[contains(@class,"m-1")]'):
                    m_1 = True
                else:
                    m_1 = False
            ...
            if len(MySEVISSchool.objects.filter(campus_id=int(id))) < 1:
                MySEVISSchool(
                    # let's try not to crawl that site again!
                    raw_html=content,
                    name=school_name,
                    campus=campus,
                    campus_id=int(id),
...
class MyBaiduImageCrawler():
    ...
    def parser(self, keyword, save_to, min_width=800):
        url = 'http://image.baidu.com/i?tn=baiduimagejson&ipn=r&ct=201326592&cl=2&lm=-1&st=-1&fm=result&fr=&sf=1&fmq=1430450474499_R&pv=&ic=0&nc=1&z=&se=1&showtab=0&fb=0&width=800&height=&face=0&istype=2&ie=utf-8&rn=20&word=%s' % keyword.encode(
            'utf-8')
        self.logger.info(url)
        ...
        blacklist = ['gkcx']
        for d in data:
            if 'width' in d and d['width'] < min_width:
                continue
            link = d['objURL']
...
class MyGKPPlanCrawler():
    ...
    def wait_for_presence(self, by_what, val, wait_length=15):
        WebDriverWait(self.agent, wait_length).until(
            EC.presence_of_element_located((by_what, val)))

    def wait_for_invisible(self, by_what, val, wait_length=15):
        WebDriverWait(self.agent, wait_length).until(
            EC.invisibility_of_element_located((by_what, val)))
    ...
    def parser(self, id):
        ...
        elem = self.agent.find_element_by_xpath(
            '//div[contains(@class,"schoolName")]/strong')
        ...
        for cat in [1, 2]:
            ...
            # for state in
            # self.agent.find_elements_by_xpath("//*[@data-id='2']"):
            for state in keys:
                ...
                self.wait_for_presence(
                    By.XPATH, "//*[contains(text(),'loading')]", 30)
                ...
                tds = self.agent.find_elements_by_xpath(
                    '//*[@class="claimContent"]/descendant::td')
...
def gkp_plan_consumer(id):
    # has to use Selenium for this one
    http_agent = SeleniumUtility(use_tor=True)
