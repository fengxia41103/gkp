    ...
    def post(self, request):
            ...
            for p in MyProvince.objects.all():
                if p.province in province:
                    ...
                    if city:
                        ...
                        city_obj = MyCity.objects.filter(
                            city=p.province, province=p)

                    if city_obj:
                        user_profile.city = city_obj[0]
                    break

        if student_type:
            user_profile.student_type = student_type
        if score:
            user_profile.estimated_score = int(score)
        if degree_type:
            user_profile.degree_type = degree_type
        user_profile.save()
        ...
        if tags:
            ...
            for t in tags.replace(u'，', ',').split(',')[:10]:
                if not t.strip():
                    continue
                tagged_item, created = MyTaggedItem.objects.get_or_create(
                    tag=t[:16])
...
class UserBookmark(TemplateView):
    ...
    def post(self, request):
        ...
        if request.POST['action'] == '1':  # toogle bookmark
            if school in user_profile.school_bookmarks.all():
            ...
            user_profile.school_bookmarks.remove(school)
            if school in user_profile.school_xouts.all():
                user_profile.school_xouts.remove(school)
            else:
...
def import_admission_by_school(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            MyCrawler().admission_by_school_persist(
                [[a.decode('UTF-8') for a in c.split('\t')] for c in content])
...
def import_admission_by_major(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            content = request.FILES['myfile'].read().split('\n')
            MyCrawler().admission_by_major_persist(
                [[a.decode('UTF-8') for a in c.split('\t')] for c in content])
...
class MyAdmissionBySchoolAdd (CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(
            MyAdmissionBySchoolAdd, self).get_context_data(**kwargs)
...
class MyAdmissionBySchoolEdit (UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(
            MyAdmissionBySchoolEdit, self).get_context_data(**kwargs)
...
class MyAdmissionBySchoolDelete (DeleteView):
    ...
    def get_context_data(self, **kwargs):
        context = super(
            MyAdmissionBySchoolDelete, self).get_context_data(**kwargs)
...
class MyAdmissionByMajorEdit (UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(
            MyAdmissionByMajorEdit, self).get_context_data(**kwargs)
        context['title'] = u'编辑fenshuxian(admission)'
        context['list_url'] = reverse_lazy('admission_major_list')
        return context
...
class MyAdmissionByMajorDelete (DeleteView):
    ...
    def get_context_data(self, **kwargs):
        context = super(
            MyAdmissionByMajorDelete, self).get_context_data(**kwargs)
        context['title'] = u'删除fenshuxian(admission)'
        context['list_url'] = reverse_lazy('admission_major_list')
        return context
...
class MyMajorDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        keywords = ','.join(
            [''.join([j.title, j.location, j.req_degree]) for j in jobs])
        context['keywords'] = pynlpir.get_key_words(
            keywords, max_words=25, weighted=True)
        pynlpir.close()
    ...
    def post(self, request, pk):
        ...
        related_schools = MySchool.objects.filter_by_user_profile(
            self.request.user).filter(id__in=[s.id for s in related_schools])
        ...
        for val in search_value.split(','):
            related_schools = related_schools.filter(
                Q(city__icontains=val) | Q(name__icontains=val))
...
class MyMajorSchoolDetail(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        major_admissions = MyAdmissionByMajor.objects.filter(
            school=school, major=major).order_by('-year', 'province')

        context['school'] = school
        context['major'] = major
        context['admissions'] = major_admissions
        context['other_majors'] = filter(
            lambda x: x.degree_type == user_profile.degree_type, school.majors.all())
...
class MyMajorRelatedSchools(TemplateView):
    ...
    def post(self, request):
        ...
        # filtered by user profile
        if filtered_by_profile:
            related_schools = MySchool.objects.filter_by_user_profile(self.request.user).filter(
                id__in=[s.id for s in related_schools]).order_by('province', 'city')
        else:
            related_schools = MySchool.objects.filter(
                id__in=[s.id for s in related_schools]).order_by('province', 'city')
...
class MySchoolDetailMajor(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for cat, by_cat_list in groupby(majors, lambda x: x.subcategory.category):
            major_by_cat[cat] = sorted(
                list(by_cat_list), lambda x, y: cmp(x.subcategory, y.subcategory))
...
class MySchoolDetailAdmission(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for year, admission_by_year_list in groupby(school_admission, lambda x: x.year):
            school_admission_by_year[year] = sorted(
                list(admission_by_year_list), lambda x, y: cmp(x.category, y.category))
...
class MySchoolDetailRelatedSchools(TemplateView):
    ...
    def post(self, request):
        ...
        # related list
        my_rank = MyRank.objects.select_related().get(
            rank_index=-1, school=school)

        # TODO: this is an expansive query. Adding "rank" column to DB index
        # would help
        city = school.city
        tmp = MyRank.objects.filter(school__city=city).filter(
            rank_index=-1, rank__gte=(my_rank.rank - 100), rank__lte=(my_rank.rank + 100))[:10]
        related_schools = [
            a for a in reversed(sorted(tmp, lambda x, y:cmp(x.rank, y.rank)))]
...
class MySchoolEchartMapFilter(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        result = {}
        if self.request.user.is_authenticated():
            schools = MySchool.objects.filter_by_user_profile(self.request.user).values(
                'id', 'name', 'province__id', 'province__province', 'city__city')
            echart_data = MySchool.objects.filter_by_user_profile(self.request.user).values(
                'province__province', 'province__id').annotate(count=Count('province__province'))
        else:
            # this is when browsing anonymously
            schools = MySchool.objects.values(
                'id', 'name', 'province__id', 'province__province', 'city__city')
            echart_data = MySchool.objects.values(
                'province__province', 'province__id').annotate(count=Count('province__province'))
        ...
        try:
            context['echart_data_min'] = min([a['count'] for a in echart_data])
        except:
            context['echart_data_min'] = 0
        try:
            context['echart_data_max'] = max([a['count'] for a in echart_data])
        except:
            context['echart_data_max'] = 0
...
class MySchoolRank(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        user_profile = MyUserProfile.objects.get(owner=self.request.user)
        tags_related_majors = [
            major for tag in user_profile.tags.all() for major in tag.mymajor_set.all()]

        # this is how we measure a TOP 10!
        context['ranks'] = MyRank.objects.filter(
            rank_index=-1, school__in=schools).order_by('-rank')[:top_count]
        return context
...
class MySchoolMajorsFilterByTags(TemplateView):
    ...
    def post(self, request):
        ...
        if user_profile.tags.all():
            tags_related_majors = [
                major for tag in user_profile.tags.all() for major in tag.mymajor_set.all()]
            related_majors = set(tags_related_majors).intersection(
                rank.school.majors.all())
        else:
            related_majors = rank.school.majors.all()
...
class MySchoolMapFilter (TemplateView):
    ...
    def post(self, request):
        ...
        visible_html = visible_template.render(
            Context({'objs': filtered_objs, 'total': len(filtered_objs)}))
...
class AnalysisSchoolDetailAJAX(TemplateView):
    ...
    def post(self, request):
        ...
        for cat in helper.filters['cats']:
            ...
            tmp = MyAdmissionBySchool.objects.filter(
                school__in=objs).values_list('max_score', flat=True)
            try:
                max_score = max(tmp)
                max_score = MyAdmissionBySchool.objects.filter(
                    school__in=objs).filter(max_score=max_score)[0]
            except:
                max_score = None

            tmp = MyAdmissionBySchool.objects.filter(
                school__in=objs).values_list('min_score', flat=True)
            try:
                min_score = min(tmp)
                min_score = MyAdmissionBySchool.objects.filter(
                    school__in=objs).filter(min_score=min_score)[0]
            except:
                min_score = None
...
class AnalysisSchoolByCity(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # trains
        context['trains'] = MyTrainStop.objects.filter(
            stop_name__icontains=city.city)
        return context
...
class AnalysisMajorByCategory(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['total_majors'] = MyMajor.objects.filter(
            subcategory__in=cat.subs.all()).count()
        return context
...
class AnalysisMajorBySubcategory(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        jobs = [job for major in cat.mymajor_set.all()
                for job in major.jobs.all()]
        context['jobs'] = random.sample(jobs, min(len(jobs), 100))
        return context
...
import pynlpir
# http://pynlpir.readthedocs.org/en/latest/api.html#module-pynlpir.nlpir
...
class MyTrainRoute(TemplateView):
    ...
    def post(self, request):
        ...
        if start_province:
            province = MyProvince.objects.filter(
                province__icontains=start_province)
        else:
            province = None

        if province and len(province) and start_city:
            start = MyCity.objects.filter(
                city__icontains=start_city, province=province[0])
        elif start_city:
            start = MyCity.objects.filter(city__icontains=start_city)
        else:
            start = None
        ...
        if start and len(start):
            start = start[0]
            all_stops = MyTrainStop.objects.filter_by_start_end(
                start.city, dest.city)
            train_ids = set(all_stops.all().values_list('train_id', flat=True))

            # group train for display
            trains = [all_stops.filter(train_id=id).order_by(
                'stop_index') for id in train_ids]
            group_by_category = {
                'G': [], 'D': [], 'Z': [], 'T': [], 'K': [], 'MISC': []}
            categories = group_by_category.keys()
            for train in trains:
                if train[0].category in categories:
                    key = train[0].category
                else:
                    key = 'MISC'

                tmp = {'stops': train}
                for stop in train:
                    if start.city in stop.stop_name:
                        tmp['start'] = stop
                    elif dest.city in stop.stop_name:
                        tmp['dest'] = stop
                segment = (
                    tmp['dest'].arrival - tmp['start'].arrival).total_seconds()
